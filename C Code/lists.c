...
list *newList(item d){
  ...
  xs->first = preNode;
  xs->last = preNode;
  preNode->before = xs->last;
  preNode->next = xs->first;
  return xs;
}
...
void freeList(list *l){
  ...
  while ((l->currentNode->before)!=l->last){
    node *x = l->currentNode->before;
    free (l->currentNode);
    l->currentNode = x;
  }
  free(l->first);
...
void insertF(list *l, item x){
  // if (endB(l) == false){
  //   node *insertF = malloc(sizeof(node));
  //   insertF->nodeVal = x;
  //   insertF->before = l->currentNode->before;
  //   l->currentNode->before = insertF;
  //   insertF->next = l->currentNode->next;
  // }
}
...
void insertB(list *l, item x){
  // if (endF(l) == true){
  //   node *insertF = malloc(sizeof(node));
  //   node *xN = l->currentNode;
  //   l->currentNode->before->next = insertF;
  //   insertF -> next=xN;
  //   insertF -> nodeVal = x;
  // }
}
...
item getF(list *l){
  if (endF(l) == false) return l->currentNode->nodeVal;
  else return l->defaultI;
}
item getB(list *l){
  if (endB(l) == false) return l->currentNode->nodeVal;
  else return l->defaultI;
}
