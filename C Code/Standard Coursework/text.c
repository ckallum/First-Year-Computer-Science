...
text *newText(char *s) {
  ...
  t -> capacity = 24;
  while (t -> capacity <= strlen(s)){
    t -> capacity = t -> capacity * 2;
  }
  t -> content = malloc((strlen(s)+1) * sizeof(char));
  strcpy(t->content, s);
  return t;
}
...
void append(text *t1, text *t2) {
  while (strlen(t2->content)+strlen(t1->content) +2 > t1->capacity){
...
text *slice(text *t, int i, int j) {
  int k = i;
  char charSlice[strlen(t->content)+1];
  for (i = k; i < j; ++i){
    charSlice[i-k] = t->content[i];
  }
  charSlice[i] = '\0';
  return newText(charSlice);
}
